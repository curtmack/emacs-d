    ...
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                ...
                (linum-relative-current-face (,@fmt-stnd ,@fg-green ,@bg-base02)) ; Relative Linum
                ...
                (sp-wrap-overlay-opening-pair (,@fg-green))
                (sp-wrap-overlay-closing-pair (,@fg-red))
                ...
                ;; anzu
                (anzu-match-1 (,@fg-base3 ,@bg-cyan))
                (anzu-match-2 (,@fg-base3 ,@bg-green))
                (anzu-match-3 (,@fg-base3 ,@bg-yellow))
                (anzu-mode-line (,@fmt-bold ,@fg-magenta))
                (anzu-mode-line-no-match (:inherit anzu-mode-line))
                (anzu-replace-highlight (:inherit query-replace))
                (anzu-replace-to (,@fg-red))
                ;; geiser
                (geiser-font-lock-autodoc-current-arg (,@fg-orange))
                (geiser-font-lock-autodoc-identifier (,@fg-cyan))
                (geiser-font-lock-error-link (:inherit link))
                (geiser-font-lock-doc-button (:inherit custom-button))
                (geiser-font-lock-doc-link (:inherit link))
                (geiser-font-lock-doc-title (,@fmt-bold))
                (geiser-font-lock-repl-input (,@fmt-bold))
                (geiser-font-lock-repl-prompt (,@fg-cyan))
                ;; haskell
                (haskell-keyword-face (,@fg-cyan))
                ;; cperl
                (cperl-array-face (,@fg-yellow ,@bg-base02))
                (cperl-hash-face (,@fg-red ,@bg-base02))
                (cperl-nonoverridable-face (,@fg-green)))))))
